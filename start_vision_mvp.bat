...
start "Worker Bandejas" cmd /k "%ACTIVATE_CMD% cd code && watchdog.bat worker_bandejas.py"

echo [2/3] Levantando Worker de Postura...
start "Worker Postura" cmd /k "%ACTIVATE_CMD% cd code && watchdog.bat worker_postura.py"

echo [3/3] Levantando Dashboard Streamlit...
start "Dashboard Vision-MVP" cmd /k "%ACTIVATE_CMD% cd code && streamlit run dashboard.py"
